...
export function ScalesPage() {
  ...
  return (
    <AppShell title="Escalas clínicas">
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-midnight-950">Escalas clínicas</h2>
            <p className="text-sm text-slate-500">
              Catálogo disponible para registrar puntajes, con el cuestionario descargable en inglés y español.
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="secondary" onClick={handleDownloadGuide} loading={downloadingGuide}>
              Guía protocolar (PDF)
            </Button>
            {canEdit && <Button onClick={openCreate}>Nueva escala</Button>}
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-3xl bg-white shadow-card">
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <Table
                headers={['Código', 'Nombre', 'Descripción', 'Cuestionario', ...(canEdit ? [''] : [])]}
                rows={items.map((item) => (
        ...
        >
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-slate-200 p-4">
              ...
              <Input label="URL" value={form.pdf_url} onChange={(e) => setForm({ ...form, pdf_url: e.target.value })} />
              <div className="mt-3">
            ...
            <div className="rounded-2xl border border-slate-200 p-4">
              ...
              <Input label="URL" value={form.pdf_url_es} onChange={(e) => setForm({ ...form, pdf_url_es: e.target.value })} />
              <div className="mt-3">
                <label className="mb-1.5 block text-sm font-medium text-slate-600">O subí el archivo</label>
                <input
                  type="file"
