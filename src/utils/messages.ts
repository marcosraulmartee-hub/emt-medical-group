...
import { minutesToLabel, timeToMinutes } from './timeGrid'
...
export function buildAppointmentConfirmMessage(appointment: Appointment): string {
  ...
  return `Hola ${appointment.patient?.full_name ?? ''}, te escribimos de EMT Medical Group para confirmar tu cita del ${dateLabel} a las ${minutesToLabel(timeToMinutes(appointment.start_time))}${clinicianLine}. ¿Confirmas tu asistencia?`
}
...
export function buildDailyAgendaMessage(date: Date, appointments: Appointment[]): string {
  ...
  const lines = active.map((a) => {
    const time = minutesToLabel(timeToMinutes(a.start_time))
